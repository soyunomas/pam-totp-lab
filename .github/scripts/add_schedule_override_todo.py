...
priority_anchor = "| 10 | `pam_totp_duress` | Investigación | Media | Sí |\n"
priority_row = "| 11 | `pam_schedule_totp_override` | Alto | Baja-media | Sí |\n"
if priority_anchor not in text:
    raise SystemExit("priority anchor not found")
text = text.replace(priority_anchor, priority_anchor + priority_row, 1)
...
text = text.replace(marker, section + "---\n\n## Ideas descartadas inicialmente", 1)

phase_anchor = "3. `pam_totp_duress`\n"
phase_entry = "4. `pam_schedule_totp_override`\n"
if phase_anchor not in text:
    raise SystemExit("phase anchor not found")
text = text.replace(phase_anchor, phase_anchor + phase_entry, 1)

result_anchor = "pam_totp_duress\n    └── detección local de coacción\n"
...
text = text.replace(result_anchor, result_anchor + result_entry, 1)
